...
export class UsersDAO implements UsersAdapter {
  ...
  private findUserByEmailStatement = q.users
    .findFirst({
      where: (users, { eq }) => eq(users.email, sql.placeholder('email')),
    })
    .prepare('findUserByEmail')

  readUserByEmail = wrap(
    async (email: string): Promise<AdapterReturn<User | undefined>> => {
      const user = await this.findUserByEmailStatement.execute({ email })

      return { data: user, error: null }
    },
  )

  updateUserImage = wrap(
  ...
  private userWithSubmissionIdStatement = q.users
    .findFirst({
      columns: {
        id: true,
      },
  ...
  hasUserSubmitted = wrap(
    async (
      id: string,
    ): Promise<AdapterReturn<[boolean, string | undefined]>> => {
      const userWithSubmissionId =
        await this.userWithSubmissionIdStatement.execute({
          id,
        })
